...
let reset () : unit =
  Array.iter (fun row -> Helpers.array_map_modify (fun _ -> []) row)
             world
...
let has_obstacles (p:int*int) : bool =
  List.fold_left (||) false (List.map (fun o -> o#is_obstacle) (get p))

(** True if there are any obstacles in the path. *)
let path_has_obstacles (ps:(int*int) list) : bool =
  List.fold_left (||) false (List.map has_obstacles ps)
